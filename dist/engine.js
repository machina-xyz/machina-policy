...
export class PolicyEngine {
    ...
    async evaluate(action) {
        ...
        const matchedRules = [];
        let ruleDecision = "DENY"; // default-deny
    ...
    evaluateCondition(condition, action) {
        ...
        switch (condition.op) {
            ...
            case "!=": return val !== condition.value;
            case ">": return typeof val === "number" && val > condition.value;
            case ">=": return typeof val === "number" && val >= condition.value;
            case "<": return typeof val === "number" && val < condition.value;
            case "<=": return typeof val === "number" && val <= condition.value;
            ...
            case "matches": return typeof val === "string" && new RegExp(condition.value).test(val);
     ...
     */
    submitVote(approvalId, vote) {
        const request = this.pendingApprovals.get(approvalId);
        if (!request || request.status !== "pending")
            return null;
        // Check voter is authorized
        if (!request.approvers.includes(vote.voterId))
            return null;
        // Check not expired
